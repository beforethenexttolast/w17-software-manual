...
| A | mapper DS4 host pre-check tool (`tools/host-precheck/ds4_precheck.sh`, real hot-plug code path) | offline/host-prechecks (mapper) · 1f20de5 | A4 (dry-run SKIP clean) | built · awaiting owner CONNECT NOW · in the D-6 grant scope (granted, not yet executed) |
| A | GS race-day timing observability: 5 `W17T` structured lines (press, spawn, link claim, late mirror claim, stop) + stamped WS3 probe log + monotonic `m`; 1693 tests | offline/raceday-timing-logs (GS) · 2f2690a | G3 → R-G(7) → FIX-G → V-G PASS | VERIFIED · **in the D-6 grant scope (granted 2026-09-05, not yet executed)** |
| B | Measurement sitting pack: runbook (81 rows, 6 stations, coordinate/sign block, stop rules), record sheets (254 cells; `param` column), generator | offline/measurement-sitting (3d) · 98e27a7 | B1 → R-B FIX_REQ → FIX-B → V-B PASS → FIX-B2 → V-B2 PASS | VERIFIED · **delivered to owner** · in the D-6 grant scope (granted, not yet executed) |
| B | CAD prep: `ingest_measurements.py` (26 tests; 35/35 §9 params matched, exit 1 on no-match), C-1.stl exported, COUPON_PRINT_PLAN, FIT_PRINT_LADDER | offline/cad-prep (3d) · 988cf17 | B2 → R-B → FIX-B → V-B → FIX-B2 → V-B2 PASS | VERIFIED · C-1.stl delivered · in the D-6 grant scope (granted, not yet executed) |
...
| — | Doc defects from the gate-card work (7) | offline/docfix-gate-citations (cf) 1d17c6b + offline/docfix-sequence-manual (ws) 30ce591 | DOCFIX-1 → V-DOC 7/7 | VERIFIED · ws half integrated · cf half in the D-6 grant scope (granted, not yet executed) |
