...
`2026-09-05_offline_decision_round_1.md`:19-23 and `bench-gates/MISSING_THRESHOLDS.md`:35.
...
| **N4** | NON-BLOCKING | *"DERIVED = 1800 mA … the conservative choice"* | **P(5) = 1510 mA** (the worst applicable cited row). **1800 mA moves to never-exceed**, cited as the module's own **supply-capability requirement** (§6.2.1, *"Peak current ≥1800mA"*) alongside the §1.3 headline. The word "conservative" is dropped — for a limit, higher is *less* protective |
...
180, **188 mA** at the shipped cap 110), the BL-M8812EU2 Wi-Fi module (S5 — **1510 mA** peak on
its worst applicable cited row, the largest single car-side 5 V load in the project and
previously undocumented), the MAX98357A (S4 — **3.35 mA** quiescent max, and **≥ 640 mA** when
...
- Rails: **A (clean)** = BL-M8812EU2 Wi-Fi, camera 5 V, ESP32 #1, ESP32 #2, RP1, WS2812 strip,
  ...
  (`[ws]/w17-pdb-build-and-connector-guide.md`:51-53 — the **sole** authority for the MAX98357A
  and the A3144, because `[fw]/docs/D8_BENCH_BRINGUP.md`:47's rail list omits both.)
- **Rail B's documented voltage band is 5–6 V, not 5 V**: *"Confirm BEC#1 ≈ 5 V, BEC#2 ≈ 5–6 V
  under a light load"* (`[fw]/docs/D8_BENCH_BRINGUP.md`:55), carried as a PASS criterion on
  `[ws]/bench-gates/BG-04_d8_bench_bringup.md` (Phase 1 row of its PASS/FAIL table) and in the
  manual (`[ws]/learning-manual/03_hardware_and_electronics_basics.md`:39). Every servo figure
...
- **The ESC's +5 V BEC red wire is CUT and insulated** — signal + GND only. The ESC's 4 A BEC
  ...
  (`[ws]/w17-pdb-build-and-connector-guide.md`:80, :162-165;
...
- Bulk capacitance, and **which side of the UBEC it is on**: **C1 1000 µF across Rail B
  ...
  **both on the 5 V side, downstream of the switching UBECs**
  (`[ws]/w17-pdb-build-and-connector-guide.md`:119-120; `[fw]/docs/00_BUILD_SHEET.md`:36
...
- The ESC's 12 AWG direct feed to batt+ is part of **A2 gate S6**
  (`[ws]/w17-pdb-build-and-connector-guide.md`:167-168). Separating it is a topology change,
...
```
  BL-M8812EU2   1510 mA   [datasheet, IPeak of the worst applicable cited row]
  ...
  ------------------------
              >= 2710 mA  =  >= 54 % of UBEC A's 5 A rating
```
...
`BG-08_dim_light_halo.md`:110 and `[sl]/docs/SIMULATION.md`:49-53, now with one side of the
...
5. **Q5 — At what RF power / mode will the BL-M8812EU2 actually run on the car, so the right row
   ...
   tight.* Absent an answer, **P(5) = 1510 mA** stands as the worst applicable cited row. Note
   separately that the module's datasheet **requires the supply to be capable of ≥ 1800 mA peak**
   (§6.2.1) — that is a never-exceed / supply-sizing figure, not a limit to dial in.
...
```markdown
| L11 | BL-M8812EU2 Wi-Fi module: peak on its worst applicable cited row / rated supply capability | **1510 mA peak** (HT20 MCS8 TX @ 27.5 dBm, 2TX) / supply must deliver **≥ 1800 mA peak** | B-link *BL-M8812EU2 datasheet* V1.0, §3.3 and §6.2.1 (retrieved 2026-09-06) | manufacturer datasheet |
