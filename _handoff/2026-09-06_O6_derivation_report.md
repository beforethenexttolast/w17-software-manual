...
| **D-1 / D-2 / D-3** | NOT ON THIS BRANCH | — | Three findings land in `W17_OWNER_ACTIONS.md`, which is checked out on branch `program/offline-readiness-r2` in a different working tree. **Not applied here**, to keep one session per working tree: DR2-2 still ratifies M-PEAK *including* the inrush carve-out R-O6 B4 killed; DR2-4 cites `BG-04:208` for the Phase 1 PASS row, which this branch moved to **:218**; and DR2-13 quotes a v1-era Rail-A floor, superseded twice over by §4 |
