...
- **Five one-look label checks (no power, ~5 min, phone photos are enough), each closes a Decision Round 2 item:** the bench PSU's model + its current-limit range (DR2-3) · the UBEC label (DR2-4) · the blower fan label (DR2-9) · an MG90S label if any (DR2-10) · the steering servo's case marking (DR2-12) · and the MH-ET boards' USB port type, USB-C or micro-USB (inventory vs BOM disagree).
...
- **DR2-1 FRESH PUSH GRANT (exact scope, one-time):** workspace branch **`program/offline-readiness-r2`** → `main`. It integrates (a) `offline/d4-thresholds` **c428196** — D-4 rulings applied into G-02/G-04/BG-08/BG-03/T11/MISSING_THRESHOLDS; Sonnet apply → Sonnet verify FIX_REQUIRED (1: the G-02 command wired the 150 ms *desired* figure as a FAIL bound) → fix with exit-code proof → fresh Sonnet verify PASS; (b) `offline/o6-derivation` — the O-6 derivation text into BG-03/T11–T13/BG-04 + the persisted Opus report, **only once its Opus adversarial review + re-verify PASS** (in progress; the grant request will name the exact tip); (c) this session's state-file commits. Nothing else. No nested-repo push is requested in this round.
- **DR2-2 Ratify the O-6 margin policy "M-PEAK":** for each load take the manufacturer's **Max/peak** figure (never Typ) and the LED model's **stated upper bound** (never its actual draw); add **no percentage**; inrush is excluded (C1/C2 charging pulls the supply *momentarily* into constant-current at each connect — healthy; the fault is a supply that **sits** in CC). Reason: a percentage would be an invented number; the typ→max gap is the manufacturer's own worst case and carries a citation.
...
- **DR2-4 (= Q2) Which UBEC is it — make/model, or a photo of the label?** Only "UBEC 5 A (2 pcs)" exists. Without its efficiency a 5 V rail draw cannot be turned into a 7.4 V supply limit. Unblocks S1–S9 and D8-1c's undefined "light load".
- **DR2-5 (= Q3) First energisation on the bench PSU rather than the battery (BG-03:40 permits it; D8:55 says battery), and can the ESC's 12 AWG feed be separated at the PDB for S0–S9?** A pack has no current limit at all. Unblocks S0, D8-1a, D8-1b.
- **DR2-6 (= Q4) Accept that S1 (ESP32 boards) and S2 (RP1) are *characterised* rather than pre-limited** — brought up at the supply's minimum settable limit, raised in its smallest increments until it leaves CC, that reading becoming T9/T8? MH-ET LIVE publishes no datasheet; RadioMaster publishes no current figure.
- **DR2-7 (= Q5) At what RF power / mode will the BL-M8812EU2 run on the car** (unassociated 123 mA peak · TX/RX 300 Mbps 928 mA peak · 2TX 1510 mA peak / **1800 mA headline max**)? Decides whether Rail A is comfortable or tight.
- **DR2-8 (= Q6) What `sound.volume` will ship, and is the speaker confirmed 4 Ω?** (inventory calls the impedance "a bench spec-check"). Unblocks S4's output-driven term.
- **DR2-9 (= Q7) Blower part number — a photo of the fan label.** The BOM names an "ACP2006-class" part, a class not a part.
- **DR2-10 (= Q8) Are the MG90S servos a branded, labelled part? If not, accept S7/S8 characterised as in DR2-6.** TowerPro's own page publishes no current figure.
- **DR2-11 (= Q9) Accept that pack-connect inrush (T13) is bounded by nothing citable and that the mitigation is procedural** (mate the XT90-S deliberately and fully, observe, any spark/heat/tick = stop)?
- **DR2-12 (= Q10) Confirm the steering servo's case marking (DS3235 / DS3235-180 / DS3235SG).** The only DS SERVO document found lacks the "SG" suffix; if "SG" is a different electrical build the 1.9 A stall ceiling could be non-conservative.
- **DR2-13 O-7 shipping question:** keep the shipped soundlight `maxBrightness = 110` until the halo gate BG-08 judges, with 180 as the ceiling the bench may raise to — **or** move the shipped value to 180 now on its own reviewed soundlight branch (needs its own grant)? Cost of 180 on Rail A: **+360 mA** modelled worst case (180 → 540 mA), **+205 mA** on the real all-amber state (103.5 → 308.2 mA); Rail A's admissible-only floor is already **2363 mA of 5 A** with the two boards, RP1 and camera still unquantified. **Director recommendation: keep 110 shipped until BG-08; no firmware change now.**
- **Doc conflicts surfaced by the derivation (no decision needed, recorded for the fix queue):** BG-03:40 "bench PSU *or* battery" vs `D8_BENCH_BRINGUP.md`:55 battery-only (resolves with DR2-5); `HARDWARE_INVENTORY.md`:96 says the MH-ET boards are USB-C, `bill_of_materials_v2.md`:66 says micro-USB (resolves with the look below); cf `ci.yml` comment still says 356 native tests, current count 360.
