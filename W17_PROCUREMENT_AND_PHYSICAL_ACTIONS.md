...
| 14 | **Temperature probe / thermocouple — or a multimeter with a K-type input** | A contact probe or K-type bead thermocouple for **air** and **surface** temperature. **Check the meter first:** many multimeters include a temperature function, and the owner already needs a multimeter (row 4) — so this is a question before it is a purchase. **An IR (non-contact) gun reads surfaces, not air:** it is acceptable **only** for the one-directional surface proof-of-violation reading, never for the ambient criterion. If an IR instrument is what exists, confirm its distance-to-spot ratio gives a spot smaller than the small heatsink face | **DR2-14's thermal validation at first power has no instrument.** The BL-M8812EU2 datasheet rates the module for an **ambient −20 … +70 °C** (§1.3, §3.1) and requires customer-added cooling (Tj < 125 °C), but publishes **no thermal resistance, no case-temperature limit and no dissipation figure** — so no surface PASS temperature is derivable and the step is **evidence recording**, whose two usable readings are *ambient* (a probe) and *surface* (either instrument). Without an instrument the step produces nothing | BG-03 S5's "Thermal check at S5"; the DR2-14 thermal validation the owner ruled must still be performed at first power | `_handoff/2026-09-06_O6_addendum_report.md` §3 (derivation, and the workspace grep that found **zero** temperature instruments anywhere); `2026-09-06_offline_decision_round_2.md` **DR2-14**; `W17_OWNER_ACTIONS.md` **DR3-3** | No — any contact probe / K-type bead, or the temperature function of a meter already owned | **unknown — CHECK THE MULTIMETER FIRST.** Not a purchase until row 4's meter is confirmed to have neither a temperature function nor a bead probe |
