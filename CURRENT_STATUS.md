...
_Last updated: **2026-09-06 night (OFFLINE READINESS — DR3 one-time grant EXECUTED and CONSUMED: workspace main = 86da42c (program/offline-readiness-r3 ff, Decision Round 2 applied + the photo/label intake + O-6 addendum v2) and control-fw main = 631dee2 (docs-only: ci.yml 356 → 360, BOM:22 AP → station; CI run 34054956474 green); landing record `W17_OFFLINE_READINESS.md` §1c; PUSH GRANT CLOSED; Decision Round 3 RULED by the owner in the same message — persisted + applied + reviewed on `program/offline-readiness-r4` — `git rev-parse --short program/offline-readiness-r4` is authoritative, NOT pushed, fresh grant needed; DR3-3 is the one open owner question; nested nit branches cf `offline/docfix-atlas-wifi-station` 98ec2ef and 3d `offline/docfix-placement-ufl` 88c9a61 VERIFIED, unpushed)** — phase authority:
...
control-fw main `1d17c6b` (4 citation fixes; CI 34025339129 green incl. link2-drift); soundlight main `7220c08` (unchanged — DR2-13 RULED 2026-09-06: shipped
`maxBrightness` 110 KEPT, 180 is the owner-approved ceiling only, no firmware-change branch; any raise = evidence + reviewed change + fresh grant); ground-station main `809976c`
...
ingest/coupons; render 17/0; 26 ingest tests). **PUSH GRANT CLOSED — D-6 and DR2-1 were each one-time and are both CONSUMED; any new branch needs a FRESH grant.** **Since then (same evening), on workspace branch `program/offline-readiness-r3` (`git rev-parse --short program/offline-readiness-r3` is authoritative; NOT pushed) and cf branch `offline/docfix-ci-count-and-wifi-topology` 631dee2 (NOT pushed; `ci.yml` 356 → 360, BOM:22 AP → station; Sonnet fresh verify PASS):** Decision Round 2 rulings persisted verbatim (`2026-09-06_offline_decision_round_2.md`) and applied into the cards (Sonnet apply → Opus review 3B/9N → fix → Sonnet verify PASS); **`W17_OWNER_PHOTO_LABEL_INTAKE.md`** = the ONE no-power photo/label packet (7 items) on which DR2-3/4/6/9/10 and the speaker/heatsink residuals wait; O-6 addendum for the photo-independent items (P(5) stays 1800 mA; amp figure relabelled as the 12 dB-gain spec point; Rail A = sizing sum 2628 mA at cap 110 / 3000 mA at 180; thermal check at S5 is evidence recording with no invented limit; datasheet §4 Target TX Power = L16) → Opus review 3B/10N → fix+apply → Opus fresh verify 1B (shifted cite) → fix → scoped verify; **Decision Round 3 — since RULED (2026-09-06 night) and applied on `program/offline-readiness-r4`**: DR3-1 30-min streaming soak = the S5 thermal-soak characterisation, re-anchored after review to the first camera bring-up session (the only place shipped-mode streaming is reachable), INCOMPLETE until run; DR3-2 9 dB CONFIRMED, pre-solder GAIN-net resistance check before soldering; DR3-3 temperature instrument = the ONE open owner check; plus the 9 dB derivation (±1.6 A AMR as a damage-threshold never-exceed, three typ relabels, ISTNDBY 400 µA, `P(4)` still BLOCKED) — r4 NOT pushed, fresh grant needed; nested nit branches cf 98ec2ef + 3d 88c9a61 VERIFIED, unpushed; procurement BUY row 6 (≥ 32×32 mm heatsink, DR2-14) + CHECK row 14 (temperature probe). Handoff: `NEW_SESSION_HANDOFF.md` on r3. Landed under DR2-1 (was the afternoon's work on **`program/offline-readiness-r2`**, now on main): D-4 thresholds applied into
...
limit and the UBEC's make/model — plus manufacturer figures now cited (Wi-Fi module 1800 mA max, servo stall 2.1 A at 6 V, amp **≥ 640 mA — a lower bound resting on a *typ* 3.2 W guidance figure at the 12 dB spec point, a condition the car does not ship; the amp has no derivable upper bound, so the Rail-A sum has no defensible upper value and remains a mixed-direction sizing sum** — not a "floor" — of **2628 mA ≈ 53 %** of 5 A at the shipped cap 110 and **3000 mA = 60 %** at the 180 ceiling; relabelled 2026-09-06 evening by the O-6 addendum v2 and re-labelled 2026-09-06 night by the DR3-2 derivation v2, which **refused to substitute the datasheet's ±1.6 A absolute maximum** for the amp term: that is a damage threshold, not a load, and the part cannot draw it into a 4 Ω or 8 Ω speaker at 5 V). **Decision Round 2 is OPEN** in `W17_OWNER_ACTIONS.md` (DR2-1 grant for r2 … DR2-14 Wi-Fi heatsink 28×28 vs ≥ 32×32). **No hardware; nothing
...
**`25bf5eb`** (330 native, five envs), soundlight **`63e8256`** (137), GS **`3719592`**
(1447), iPhone **`862aeb0`** (74), mapper `w17-headtrack` **`9cb501e`** (180),
design-system `26ec870`, 3d-codex `0386b2f`. Open branches exactly `u4-arbiter`
(`93be341`, R-review + bench gated) and `codex-wip-vr-calibration` (`f0e2c82`, digest
delivered — keep-as-reference, adaptation cautions in packet §5). **No agents in flight;
all builder/reviewer worktrees removed; every repo `worktree prune`d**; the stray harness
worktree under `.claude/worktrees/` (detached, fully merged, one untracked pre-fix
`AGENTS.md`) removed. **Date amendment:** the "2026-08-20 decision day" entry below covers
merges git-timestamped 2026-08-20 15:36 through **2026-08-21 02:20 local** — the label is
the day the decisions were taken; the hashes are authoritative. Persisted this pass:
packet §7½ renumbered (items 6–7 = D4 amendment / decision day) + **item 8** (push grant
closed, authority back to owner-call; housekeeping); packet §3/§5/§6(e) dated
supersessions (drafts landed; codex-wip digest cautions; new drift-watch notes);
Checkpoints-table rows corrected below with dated notes; `WORKSPACE_MAP.md` and the
vision's Open points relieved of stale status by dated amendment; the memory resume brief
rewritten as index + delta. **Vision done-bar (reconstructed):** 1 drive envelope /
3 sound+light / 6 dual HUD / 8 giftee-operable — SOFTWARE COMPLETE, bench-unproven;
2 showpiece finish / 4 USB-C charge flap / 5 DRS mechanics — Codex-side mechanical;
7 head-tracked gimbal — code parked on `u4-arbiter`, FIRST_ACTIVE stays NO-GO (R1–R16 +
bench evidence). **Owner residue:** shopping only (5.8 GHz AP-capable adapter, powered
hub, SP3T switch, ELRS TX label, TX16S check) + optional shipped-tune compile pin (sl).
**Next bounded task for a successor: NONE autonomous** — idle-verify, then act on the
owner's next input; every remaining item is owner-, Codex-, or bench-gated. **No hardware;
nothing flashed or powered; A2 stays NOT-EXECUTED, Phase B stays BLOCKED.**
Prior pass, **2026-08-20 (owner decision day — plate cleared, re-arm invariant, booklet final)** —
...
| `iPhone_rc` (Claude Code since 2026-08-17) | `862aeb0` | **Row corrected 2026-09-02 (rollover checkpoint): `main` = `862aeb0`, PUSHED, level; 74 tests; `codex-wip-vr-calibration` @ `f0e2c82` parked (digest delivered). The "(Codex)" label and the text after this sentence are as-of 2026-08-16.** VR FPV plan consolidation (H1–H11 applied; canonical contract sync revision); Batch 1 VR-calibration work remains uncommitted in its working tree |
