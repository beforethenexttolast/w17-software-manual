...
55. **Are the dim light layers visible after the brightness cap + gamma?** Measured (exact
    ...
    bright layers land at 40 (brake/amber/white). Raised 2026-09-03 (`kMinVisibleDuty = 6`,
    `w17-soundlight-fw/lib/lights/include/lights/LightRenderer.hpp:127`; constants at
    `lib/lights/src/LightRenderer.cpp:18-19,88`) from a pre-raise 1/255 duty on the same three
    quiet states — each now clears the floor on its brightest channel (6–9/255, 2.4–3.5 % duty).
    Bench: judge visibility, retune `maxBrightness`/palette if still marginal (the power budget
    has ~5× real headroom, since `valid()`
