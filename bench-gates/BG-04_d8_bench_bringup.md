...
12. **Phase 9 — link2 → board #2.** Flash soundlight; wire GPIO25 → GPIO16, **common ground**,
    115200 8N1. MAX98357A GAIN strap (9 dB, GAIN floating — CONFIRMED DR3-2 as the
    *intended* shipped configuration; **9 dB is a typ, guaranteed band 8.4 … 9.6 dB**). **The
    strap stays BLOCKED until the unpowered pre-solder GAIN check is recorded** —
    `W17_PROCUREMENT_AND_PHYSICAL_ACTIONS.md` "Physical actions that need no purchase", item 11:
    a continuity beeper cannot tell **unconnected (9 dB)** from **100 kΩ to GND (15 dB)**, so the
    check is a resistance measurement in both polarities, and **any resistor, jumper, bridge or
    trace on the GAIN net other than an open header pad means: do not solder.** WS2812 behaviours. **Cut the UART
