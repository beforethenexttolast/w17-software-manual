...
| feat/phone-live-video (iPhone) | wt-iphone-video (new) | base 2ce12ee + cherry-pick 50f25da | IMPLEMENTER slices 1–3 (Opus) in flight |
| design/phone-live-video (iPhone) | wt-iphone-video-design | 50f25da | design doc; consumed by the implementer |
| fix/sensor-honesty-and-ci (cf) | wt-cf-fix | d8c5f7c | built (351 tests, 5 envs) → REVIEW (Opus) in flight |
| fix/lights-truth-wdt-and-clamp (sl) | wt-sl-fix | 1824228 | built (149 tests) → REVIEW (Opus) in flight |
| fix/gs-packaging-and-resilience (GS A) | wt-gs-fixA | 1f2598d | built (1505 tests) → REVIEW (Opus) in flight |
| fix/gs-race-day-truth-and-lifecycle (GS B) | wt-gs-fixB (new) | base 1f2598d | IMPLEMENTER (Opus) in flight |
| fix/headless-bringup-and-link (mapper A) | wt-mapper-fixA | b071a30 | built (219 tests, -race) → REVIEW (Opus) in flight |
| fix/gamepad-hotplug-and-lint-teeth (mapper B) | — | — | NOT STARTED; depends on mapper A merged |

## 4. Failed / review-required
None failed. Every FIX_REQUIRED above has a fixer assigned. Usage-limit stops: three (2026-09-02/03), all recovered from durable state.
