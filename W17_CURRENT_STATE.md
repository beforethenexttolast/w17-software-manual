...
| fix/gs-race-day-truth-and-lifecycle (GS B) | wt-gs-fixB | 5dcdce5 (19 commits on 439f09f; 1680 tests; OD-19 refinement applied, WS3 probe comment added) | fixed (reports/ad740b14ba4f04d7f.md: patchTelemetrySource never round-trips the credential — test proven to bite first; link-not-yet on first bring-up; kill()===false → stopFailed immediately; last-3-lines exit message; smoke scenario for the single-instance lock, runs only in CI) → RE-VERIFY (Opus, VERIFY-gsB.md; in flight — drives the GRID gate with a dead radio to test whether auto-START can fire on link-not-yet) → merge (+ noControlPath citation refresh to :125-170) → push → CI |
| fix/headless-bringup-and-link (mapper A) | (worktree removed) | 6e99d51 | LANDED on w17-headtrack, pushed |
| fix/gamepad-hotplug-and-lint-teeth (mapper B) | wt-mapper-fixB | f3e5fd8 (10 commits on 6e99d51; 287 tests, -race clean; hook exercised on 9 injection shapes) | built (reports/aa8d99ee94acdeeba.md: keepalive flake fixed with an injected clock; SDL hot-plug with retire-not-close; lint teeth for W17-marked profiles; positive enum assertion in the hook; -list-devices JSON; CORS same-origin; vet chan) → ADVERSARIAL REVIEW (Opus, REV-mapperB.md) in flight → fix → verify → ff onto w17-headtrack → push → dispatch the release workflow → booklet :252/:380 + handover-checklist `w17_profile` line (workspace docs) |
