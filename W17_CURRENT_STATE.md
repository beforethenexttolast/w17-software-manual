...
| w17-ground-station | main | 698ebda | yes | 1524 / 72 files (GS A 22ce2e5; orchestrator hotfixes 0fd950a = mirror digest normalizes CRLF, 698ebda = three Windows-portable tests: fileURLToPath, explicit LF/CRLF variants, retried asar fixture removal). CI: ubuntu test + contract-mirror GREEN on both; windows-latest package-smoke RED on 22ce2e5 (CRLF digest) and 0fd950a (test portability) → run on 698ebda being watched → then record windows_amd64 digest → --require-pin |
