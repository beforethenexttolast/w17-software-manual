...
| fix/sensor-honesty-and-ci (cf) | wt-cf-fix | 88182e2 (21 commits on 58581ff; 356 tests, 5 envs) | round-1 fixes done (reports/a0247ea4fa971b25f.md) → Opus re-verify FAIL on R1 (owed row B4.4 told the operator to run the motor inside the motor-disconnected Phase-B doc — gate softening) + R2–R6 (reports/ae6f0f9378e7cadb2.md) → round 2 fixed at 39a4f3c (360 tests; B4.4 motor-free, Hall counters on the tuning `status` line, nm-output gate, dwell test; reports/a4edf0864dd899213.md) → RE-VERIFY (Sonnet) in flight → ff merge → soundlight lib/link2 re-copy on wt-sl-link2 (branch chore/link2-resync, pre-created) → push both → observe cf + sl CI |
...
| fix/gs-race-day-truth-and-lifecycle (GS B) | wt-gs-fixB | 549b192 + rebase onto 439f09f + fixes appending | review FIX_REQUIRED (reports/aa6146e2ba93e3b89.md): boundaries held (exact-equality pin on the two read-only stream RPCs); blocking = telemetry save round-trips and can drop the hotspot credential (proved), unhedged 5 s link window → false cable halt; 9 non-blocking; the 5 open questions adjudicated as OD-19 → FIXER (Opus, FIX-gsB.md) in flight → re-verify → merge (+ noControlPath citation refresh) → push |
