...
| feat/phone-live-video (iPhone) | wt-iphone-video | d2d993f (13 appended; 156 tests) | fixed (reports/a9fa2f34cfff653a4.md: gate closes the 3 evasions + pbxproj Sources-phase completeness; citations re-pointed; design banner + README rewritten; NSAllowsLocalNetworking removed and proven non-load-bearing; port-80 normalization; reload backoff; RECONNECTING copy; NO VIDEO chip hidden while live; rVFC generation guard; Debug→Drive round-trip test; live check: live-in 0.56 s, outage → placeholder ≈1.4 s, retries 1.5/1.5/3/6/10) → RE-VERIFY (Sonnet) in flight → ff merge → push → CI → GS contract re-mirror (--sync-hash = new iPhone main). Datum for GS B's fixer: mediamtx v1.9.3 with the checked-in config already advertises LAN interface addresses (GS-1 not a reachability blocker; w17-ground-station/docs/iphone_bridge_readiness.md:93-94 'localhost-only' overstates) |
